...
export async function exportHdJpg(opts: {
  ...
  const cyber = opts.theme === "cyber";
  ctx.fillStyle = cyber ? "#0a0a0f" : "#0c0b0a";
  ...
  const stats = [
    ...
    `${formatMoney(r.totalCost)} stones`,
  ...
  ctx.fillStyle = cyber ? "#e5e7eb" : "#f4f0e8";
  ctx.font = cyber
  ...
  if (opts.notes.trim()) {
    ctx.fillStyle = cyber ? "#9ca3af" : "#a3988c";
    ctx.font = cyber
  ...
  ctx.fillStyle = cyber ? "#ff0033" : "#c4a574";
  ctx.font = cyber
    ? "700 18px 'IBM Plex Sans', sans-serif"
    : "500 18px Outfit, sans-serif";
  ctx.fillText(
    cyber ? "HANZU TECH  ·  OPENTOOLS" : "RIVIÈRE",
    x,
    1540,
  );
